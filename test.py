...
class AppStateTest(AppState):
    ...
    def on_environment_reset(self, episode_recorder_dict):
        ...
        self._camera_helper.update(self._get_camera_lookat_pos()[0], dt=0)
    ...
    def _try_grasp_remote(self):
        ...
        print(f"self._held_target_obj_idx: {self._held_target_obj_idx}")
        self._remote_held_hand_idx = found_hand_idx
        print(f"self._remote_held_hand_idx: {self._remote_held_hand_idx}")
    ...
    def _update_grasping_and_set_act_hints_remote(self):
        ...
        print(f"hand_idx: {hand_idx}")
        print(f"reach_pos: {reach_pos}")
    ...
    def _get_agent_pose(self):
        ...
        # Print translation and rotation coordinates of agent
        # print(f"Agent_translation: {agent_root.translation}")
        # print(f"Agent_rotation: {agent_root.rotation}")

        return agent_root.translation, agent_root.rotation

    def _get_camera_lookat_pos(self):
        agent_pos, agent_rot = self._get_agent_pose()
        lookat_y_offset = mn.Vector3(0, 0, 0)
        lookat = agent_pos + lookat_y_offset
        # print(f"agent_pos: {agent_pos.x, agent_pos.y, agent_pos.z}")
        # print(f"agent_rot: {agent_rot}")
        # print(f"lookat: {lookat}")
        return lookat, agent_pos, agent_rot
    
    def write_to_csv(self, filename, data):
        with open(filename, 'a', newline='') as csvfile:
            fieldnames = ['timestamp', 'held_target_obj_id', 'agent_translation', 'agent_rotation', 'target_objects']
    ...
    def sim_update(self, dt, post_sim_update_dict):
        if self._app_service.gui_input.get_key_down(GuiInput.KeyNS.ESC):
            # Before closing, save the video
            vut.make_video(
                observations=self.observations,
                primary_obs="agent_0_articulated_agent_arm_rgb", # Options: 'agent_0_head_rgb', 'agent_0_head_depth', 'agent_0_articulated_agent_arm_rgb', 'agent_0_articulated_agent_arm_depth'
        ...
        lookat, agent_pos, agent_rot = self._get_camera_lookat_pos()
        ...
        # Save helpful information to csv
        data = {
            'timestamp': datetime.now(),
            'held_target_obj_id': self._held_target_obj_idx,
            'agent_translation': agent_pos,
            'agent_rotation': agent_rot,
            'target_objects': json.dumps({
                'obj1': {'pos': [], 'vel': []},
                'obj2': {'pos': [], 'vel': []}
            })
        }
        # print(f"Data for csv: {data}")
        csv_filepath = os.path.join(os.getcwd(), 'habitat_experimenting', 'testdata', f'testdata-{datetime.now().strftime("%Y-%m-%d_ %Hh%Mm")}.csv')
        self.write_to_csv(csv_filepath, data)

        # Create sensor observation and add to the list
        if self.make_video:
            sim = self.get_sim()
            self.observations.append(sim.get_sensor_observations())

        ## Print useful information from during the sim [DEPRECATED]
...
@hydra.main(
    version_base=None, config_path="config", config_name="test"
)
